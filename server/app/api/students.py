...
def getUser(username):
    ...
    if user == current_user:
        return jsonify(user.studentToDict(True))
    return jsonify(user.studentToDict())
...
def createUser():
    ...
    db.session.commit()
    response = jsonify(student.studentToDict(True))
...
def updateUser(username, response):
    if request.method == 'PUT':
        ...
        data = request.get_json() or {}
        if 'username' in data and data['username'] != student.username and Student.query.filter_by(username=data['username']).first():
            return badRequest('please use a different username')
        if 'email' in data and data['email'] != student.email and Student.query.filter_by(email=data['email']).first():
